...
| Jackson | **3.1.4** | Spring Boot 管理 | 直接依赖 `tools.jackson.core:jackson-databind`；注解包 `com.fasterxml.jackson.annotation` 由其传递提供 |
...
| JSON | Jackson | **3.1.4** | Spring Boot 管理；使用 `tools.jackson.*`，不显式声明 `jackson-annotations` |
